...
# Fit and transform the 'rukun' column using label encoder
...
for idx, row in df_grouped.iterrows():
    ...
    if id_matrix.shape[0] < sequence_length:
        ...
        y_lstm_flat = [row['physicalTiredLevel'][0]] * (id_matrix.shape[0] // sequence_length)
        ...
        for start in range(0, id_matrix.shape[0] - sequence_length + 1, sequence_length):
            ...
            y_lstm.append(row['physicalTiredLevel'][start])

y_lstm = np.array(y_lstm)

# Convert physicalTiredLevel to categorical
y_lstm_categorical = to_categorical(y_lstm - 1, num_classes=5)
...
X_train_lstm, X_val_lstm, y_train_lstm, y_val_lstm  = train_test_split(
    X_lstm_normalized, y_lstm_categorical, test_size=0.33, random_state=42
)
...
model.add(Dense(5, activation='softmax'))  # Use 5 units for the 5 classes and softmax activation
...
plt.legend()
plt.savefig('tiredness_classifier_train_vs_validation.png')
...
tabnet_history=tabnet_model.fit(X_train_tabnet, y_train_tabnet, eval_set=[(X_val_tabnet, y_val_tabnet)], patience=10, max_epochs=50)
...
# Print the available keys in history_dict
print(history_dict.keys())

# Assuming history_dict is the history attribute of the history object
train_loss = history_dict['loss']
val_accuracy = history_dict['val_0_accuracy']
...
plt.legend()
plt.show()
plt.savefig('tabnet_tiredness_classifier_train_vs_validation.png')
