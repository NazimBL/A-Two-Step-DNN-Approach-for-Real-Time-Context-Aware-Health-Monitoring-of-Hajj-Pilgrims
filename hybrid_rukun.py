...
print(df['rukun_encoded'].value_counts())
...
numerical_features = ['gsr_x', 'altitude', 'peakAcceleration', 'ibi', 'temp', 'x', 'y', 'z', 'heartRate', 'respirationRate', 'heartRateVariability']
...
for idx, row in df_grouped.iterrows():
    ...
    if id_matrix.shape[0] < sequence_length:
        ...
        y_lstm_flat = [row['rukun_encoded'][0]] * (id_matrix.shape[0] // sequence_length)
        ...
        for start in range(0, id_matrix.shape[0] - sequence_length + 1, sequence_length):
            ...
            y_lstm.append(row['rukun_encoded'][start])

y_lstm = np.array(y_lstm)

# Convert emotionalMoodLevel to categorical
y_lstm_categorical = to_categorical(y_lstm, num_classes=17)
...
X_train_lstm, X_val_lstm, y_train_lstm, y_val_lstm  = train_test_split(
    X_lstm_normalized, y_lstm_categorical, test_size=0.2, random_state=42, stratify=y_lstm_categorical
)
...
model.add(Dense(17, activation='softmax'))  # Use 5 units for the 5 classes and softmax activation
...
X_train_tabnet, X_val_tabnet, y_train_tabnet, y_val_tabnet = train_test_split(
    lstm_embeddings_reshaped, y_lstm_reshaped, test_size=0.33, random_state=42
)
